...
export function buildCodexOcrPrompt(input: {
  ...
  return [
    ...
    "Segment assets for tables, formulas, charts, code blocks, forms, illustrations, or embedded images.",
    ...
    "All bbox fields must be axis-aligned top-left coordinate boxes. Prefer coordinate_space normalized_1000 with value [x1,y1,x2,y2] on a 0..1000 page grid.",
...
export async function runCodexOcr(
  ...
  try {
    runResult = await runCodexOcrWithSdk(prompt, imageInfo.codexPath, {
      ...options,
      model,
      reasoningEffort,
      timeoutMs,
  ...
  if (options.rawResponsePath) {
    ...
    await fs.writeFile(options.rawResponsePath, `${JSON.stringify(runResult.raw, null, 2)}\n`);
  }
...
function normalizeCodexParsed(value: unknown, fallbackPageNumber: number): CodexOcrParsed {
  ...
  return {
    ...
    layout_regions: normalizeLayoutRegions(record.layout_regions),
    markdown: stringValue(record.markdown),
    page_metadata: normalizePageMetadata(record.page_metadata, fallbackPageNumber),
    warnings: uniqueStrings(Array.isArray(record.warnings) ? record.warnings.map(String) : []),
  };
}

function normalizePageMetadata(value: unknown, fallbackPageNumber: number) {
  const record = isRecord(value) ? value : {};
...
function clampBbox(raw: number[], imageWidth: number, imageHeight: number): OcrBbox | null {
...
function makeFallbackLineResults(text: string, imageWidth: number, imageHeight: number): OcrResult[] {
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  return lines.map((line, index) => {
    ...
    return {
      bbox: [0, y1, imageWidth, Math.max(y2, y1 + 1)],
      confidence: 1,
      text: line,
    };
  });
...
function toNumberArray(value: unknown): OcrBbox | null {
  if (!Array.isArray(value) || value.length < 4) return null;
  const numbers = value.slice(0, 4).map(Number);
  return numbers.every((item) => Number.isFinite(item))
    ? ([numbers[0], numbers[1], numbers[2], numbers[3]] as OcrBbox)
    : null;
}
